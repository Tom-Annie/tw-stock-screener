...
# ===== 控制列(用 persist_state 保證跨頁不重置) =====
from utils.persist import persist_state, make_persist_callback

_DEFAULT_LIST = "2330,2454,2317,2412,3008,2382,2308,1301,1303,2891"
_REFRESH_OPTIONS = [3, 5, 10, 30, 60]
_LAYOUT_OPTIONS = ["可拖拉", "經典"]

persist_state("live_watchlist", default=_DEFAULT_LIST)
persist_state("live_refresh_sec", default=5)
persist_state("live_auto_on", default=True)
persist_state("live_layout_mode",
               default="可拖拉" if HAS_ELEMENTS else "經典")

ctl_l, ctl_m, ctl_r, ctl_x = st.columns([3, 1, 1, 1])
with ctl_l:
    raw = st.text_input("監控股票代碼(逗號分隔)",
                         key="live_watchlist",
                         on_change=make_persist_callback("live_watchlist"))
with ctl_m:
    refresh_sec = st.selectbox(
        "刷新秒數", _REFRESH_OPTIONS,
        key="live_refresh_sec",
        on_change=make_persist_callback("live_refresh_sec"),
    )
with ctl_r:
    auto = st.toggle(
        "自動刷新",
        key="live_auto_on",
        on_change=make_persist_callback("live_auto_on"),
    )
with ctl_x:
    layout_mode = st.radio(
        "排版", _LAYOUT_OPTIONS, horizontal=True,
        key="live_layout_mode",
        on_change=make_persist_callback("live_layout_mode"),
    )
